...
router.post('/new',auth, upload.single('coverImageName'), async (req, res, next) => {
    ...
    if (film_search) {
        let film;
        film = await Film.findById(film_search._id)
...
router.put('/edit/:filmId',auth, upload.single('coverImageName'), async (req, res, next) => {
    if(!req.body.name||!req.body.publishDate||!req.body.description||!req.body.linkTrailer||!req.body.directorId||!req.body.categoryId|| !req.file ){
